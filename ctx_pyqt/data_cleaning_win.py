...
class Menu(QMainWindow):
    ...
    def init_ui(self):
        ...
        config_button = ButtonImage("清洗配置", 'icon/配置文件.png')
        new_button = ButtonImage("新建文件", 'icon/新建文档.png')
        for item in (
                search_button, sub_button, multi_file_button, z_score_button,
                pdf_button,
                data_check_button,save_button,as_save_button,config_button,new_button):
            item.setFixedSize(100, 100)
            item.setStyleSheet(ctx_button_style.button_style())
        # 添加到布局
        ...
        layout.addWidget(config_button, 0, Qt.AlignLeft)
        layout.addWidget(new_button, 0, Qt.AlignLeft)
